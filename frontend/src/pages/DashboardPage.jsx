...
function StatCard({ label, value, tone, icon, children }) {
  return (
    <article className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
      ...
      <strong className="block text-[28px] font-bold leading-none tracking-tight text-slate-900 tabular-nums">
        {value}
      </strong>
      {children}
    </article>
  ...
  return (
    <main className="flex min-h-screen bg-[#f8fafc] text-slate-900">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-10 flex items-center gap-4 border-b border-slate-200/80 bg-[#f8fafc]/95 px-6 py-4 backdrop-blur">
          <form className="flex min-w-0 flex-1 items-center" onSubmit={handleSearchSubmit}>
            <label className="relative flex min-w-0 flex-1 items-center">
              ...
              <input
                className="h-11 w-full rounded-full border border-slate-200 bg-white pl-11 pr-4 text-sm text-slate-700 shadow-[0_1px_2px_rgba(15,23,42,0.04)] outline-none transition placeholder:text-slate-400 focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100"
                onChange={(event) => setSearchQuery(event.target.value)}
                placeholder="Search documents, chats, or topics..."
                type="search"
                value={searchQuery}
              />
            </label>
        ...
        <section className="grid min-w-0 flex-1 gap-6 px-6 py-6 lg:px-8 lg:py-8">
          <section className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <h1 className="m-0 text-[28px] font-bold tracking-tight text-slate-900">
                {sectionCopy.title}
                {activeSection === "dashboard" ? `, ${displayName}` : ""}
              </h1>
              <p className="m-0 mt-1 text-sm text-slate-500">{sectionCopy.description}</p>
            </div>
            <div className="flex flex-wrap gap-3">
              <Link
                className="inline-flex min-h-10 items-center justify-center gap-2 rounded-lg border border-slate-300 bg-white px-4 text-sm font-semibold text-slate-700 no-underline transition hover:border-slate-400 hover:bg-slate-50"
              ...
              <button
                ...
                onClick={() => handleSectionChange("ai-workspace")}
                type="button"
              >
          ...
          <section className="grid gap-6 xl:grid-cols-[minmax(0,1.7fr)_minmax(300px,1fr)]">
            <article className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
              <header className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
                <SectionTitle icon={<DocumentFileIcon className="h-[18px] w-[18px]" />}>
                  Recent Documents
                </SectionTitle>
                <Link className="inline-flex items-center gap-1 text-sm font-semibold text-indigo-600 no-underline hover:text-indigo-700" to="/documents">
                  View All
              ...
              <div className="overflow-x-auto">
                <div className="grid min-w-[640px] grid-cols-[minmax(180px,1.8fr)_minmax(110px,1fr)_110px_80px_70px] gap-3 border-b border-slate-100 bg-slate-50/80 px-5 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500">
                  <span>Name</span>
                  <span>Subject</span>
                  <span>Date</span>
                  <span>Size</span>
                ...
                {isLoading ? (
                  <p className="px-5 py-8 text-sm text-slate-500">Loading documents...</p>
                ) : recentDocuments.length ? (
                  recentDocuments.map((doc) => (
                      <div
                        className="grid min-w-[640px] grid-cols-[minmax(180px,1.8fr)_minmax(110px,1fr)_110px_80px_70px] items-center gap-3 border-b border-slate-100 px-5 py-4 text-sm text-slate-600 transition last:border-b-0 hover:bg-slate-50/70"
                        key={doc.id}
                      >
                        ...
                        <Link className="inline-flex items-center gap-1 font-semibold text-indigo-600 no-underline hover:text-indigo-700" to="/documents">
                          Open
                  ...
                  <div className="px-5 py-10 text-center">
                    <span className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-2xl bg-blue-50 text-blue-600 ring-1 ring-blue-100">
                      <DocumentFileIcon className="h-7 w-7" />
                    </span>
                    <p className="m-0 text-sm text-slate-500">No documents yet.</p>
                    <Link className="mt-3 inline-flex items-center gap-1.5 text-sm font-semibold text-indigo-600 no-underline hover:text-indigo-700" to="/documents?upload=true">
                      <UploadIcon className="h-4 w-4" />
            ...
            <div className="grid gap-5 content-start">
              <article className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
                ...
                <div className="mt-4 grid gap-2">
                  {isLoading ? (
                    <p className="m-0 text-sm text-slate-500">Loading activity...</p>
                  ) : recentDocuments.length ? (
                    recentDocuments.slice(0, 3).map((doc, index) => (
                        <Link
                          className="flex items-center gap-3 rounded-xl border border-transparent px-2 py-2.5 no-underline transition hover:border-slate-200 hover:bg-slate-50"
                          key={`continue-${doc.id}`}
                          to="/documents"
                        >
                          ...
                          <span className="min-w-0 flex-1">
                            <strong className="block truncate text-sm font-medium text-slate-900">{doc.title}</strong>
                            <small className="text-xs text-slate-500">{formatRelativeTime(doc.updated_at || doc.created_at)}</small>
                          </span>
                    ...
                    <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50/70 px-4 py-5 text-center">
                      <span className="mx-auto mb-2 flex h-10 w-10 items-center justify-center rounded-xl bg-white text-slate-500 ring-1 ring-slate-200">
                        <ClockIcon className="h-5 w-5" />
                      </span>
                      <p className="m-0 text-sm text-slate-500">Start by uploading a document to see your recent activity here.</p>
                    </div>
                  )}
                </div>
              </article>
              ...
              <article className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {subjects.length ? subjects.map((subject) => (
                    ...
                    <div className="flex w-full items-center gap-3 rounded-xl border border-dashed border-slate-200 bg-slate-50/70 px-4 py-4">
                      ...
                      <span className="text-sm text-slate-500">No subjects yet.</span>
                    </div>
                  )}
                </div>
              </article>
